...
### 核心特点
- ✅ **纯前端处理**：所有转换在本地完成，保护隐私安全
- ✅ **完整保留结构**：完美保存文件夹层级关系
- ✅ **多浏览器支持**：兼容主流浏览器导出的书签格式
- ✅ **无数据上传**：文件无需上传到服务器
- ✅ **响应式设计**：支持桌面和移动设备
...
| 选项 | 说明 | 建议 |
|------|------|------|
| 包含元数据 | 添加生成时间、统计信息等 | 首次转换建议勾选 |
| 保留原始文件夹结构 | 保持书签的层级关系 | 需要完整结构时勾选 |
| 包含网站图标数据 | 保留书签的图标信息 | 图标不重要时可取消 |
| 扁平化输出 | 所有书签在一级，包含路径信息 | 数据分析时使用 |
...
### 核心技术
- **纯JavaScript**：无依赖，无需后端
- **DOMParser**：解析HTML书签文件
- **递归算法**：处理嵌套的文件夹结构
- **Blob API**：生成可下载的JSON文件
...
本项目采用 [MIT License](https://opensource.org/licenses/MIT) - 可自由使用、修改和分发
